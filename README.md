...
如果只启动 `npm run dev`、不启动后端，请将 `runtime-config.js` 设置为：

```js
window.ERP_API_BASE_URL = '';
```

重新刷新页面后，系统会进入本机密码模式。第一次使用时自行设置至少 4 位密码；该密码只保存在当前浏览器，不会连接 Neon 或本地后端。

需要验证真实账号登录、订单同步和权限时，再把地址改回 `http://127.0.0.1:3001`，并同时启动 `backend` 服务。
